...
public class LLVMIRGenerator {
    ...
    private void newAssign(ParseTree treeNode) {
        ...
        for (ParseTree child : treeNode.getChildren()) {
            if (
                ...
                child.getLabel().getValue() == NonTerminal.ExprArith
            ) {
                varId = newExprArith(child);
            } else if (
                child.getLabel().isNonTerminal() &&
                child.getLabel().getValue() == NonTerminal.ExprArithPrime
            ) {
                varId = newExprArithPrime(child);
            } else if (child.getLabel().getValue() == NonTerminal.Prod) {
                varId = newProd(child);
            }
        }
    ...
    private void newIf(ParseTree treeNode) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (child.getLabel().getValue() == NonTerminal.Cond) {
                condNode = child;
            } else if (
                child.getLabel().getType() == LexicalUnit.THEN &&
                i + 1 < children.size()
            ) {
                thenNode = children.get(1 + i);
            } else if (
                child.getLabel().getType() == LexicalUnit.ELSE &&
                i + 1 < children.size()
            ) {
    ...
    private String newCondImpl(ParseTree treeNode) {
        ...
        if (treeNode.getLabel().getValue() == NonTerminal.SimpleCond) {
            // CondImpl => CondAtom
    ...
    private String newCondAtom(ParseTree treeNode) {
        ...
        if (child.getLabel().getValue() == NonTerminal.CondPrime) {
            // COndAtom => | Cond |
        } else {
            // CondAtom => CondComp
    ...
    private String newExprArith(ParseTree treeNode) {
        ...
        indentLevel++;
        if (treeNode.getLabel().getValue() == NonTerminal.ExprArithPrime) {
            resultId = newExprArithPrime(treeNode);
        } else if (treeNode.getLabel().getValue() == NonTerminal.Prod) {
            resultId = newExprArithPrime(treeNode);
        } else if (child.getLabel().getValue() == NonTerminal.ExprArithPrime) {
            resultId = newExprArithPrime(child);
        } else {
            throw new RuntimeException(
                "Expected ExprArithPrime but found " +
     ...
     */
    private String newExprArithPrime(ParseTree treeNode) {
        List<ParseTree> children = treeNode.getChildren();
        String resultId = newProd(children.get(0));

        for (int i = 1; i < children.size(); i += 2) {
            LexicalUnit op = children.get(i).getLabel().getType();

            String newExprResult = null;
            if (children.get(i + 1).getLabel().getValue() == NonTerminal.Prod) {
                newExprResult = newProd(children.get(i + 1));
            } else if (
                children.get(i + 1).getLabel().getValue() ==
                NonTerminal.ExprArithPrime
            ) {
                newExprResult = newExprArithPrime(children.get(i + 1));
            } else if (
                children.get(i + 1).getLabel().getType() ==
                    LexicalUnit.VARNAME ||
                children.get(i + 2).getLabel().getType() == LexicalUnit.NUMBER
            ) {
                newExprResult = newAtomValue(children.get(i + 1));
            } else {
                throw new RuntimeException(
                    "Expected Prod but found " +
                        children.get(i + 1).getLabel().getValue()
                );
            }
    ...
    private String newProd(ParseTree treeNode) {
        List<ParseTree> children = treeNode.getChildren();
        String resultId = newAtom(children.get(0));
        ...
        for (int i = 1; i < children.size(); i += 2) {
            ...
            if (children.get(i + 1).getLabel().getValue() == NonTerminal.Prod) {
                ...
                children.get(i + 1).getLabel().getValue() == NonTerminal.Atom
            ) {
                newExprResult = newAtom(children.get(i + 1));
            } else {
                throw new RuntimeException(
                    "Expected Prod but found " +
                        children.get(i + 1).getLabel().getValue()
     ...
     */
    private String newAtom(ParseTree treeNode) {
        List<ParseTree> children = treeNode.getChildren();

        if (children.get(0).getLabel().getType() == LexicalUnit.MINUS) {
            String newExprPrimary = newAtom(children.get(1));
            String resultId = newUnamedI32Id();
            newLine(resultId + " = sub i32 0, " + newExprPrimary);
            return resultId;
        } else if (children.get(0).getLabel().getValue() == NonTerminal.Atom) {
            return newAtom(children.get(0));
        } else if (
            children.get(0).getLabel().getType() == LexicalUnit.VARNAME ||
            children.get(0).getLabel().getType() == LexicalUnit.NUMBER
        ) {
            return newAtomValue(treeNode);
        } else {
            throw new RuntimeException(
                "Unexpected node " + children.get(0).getLabel().getType()
            );
        }
    }

    private String newAtomValue(ParseTree treeNode) {
