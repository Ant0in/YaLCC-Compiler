...
public class LLVMIRGenerator {
  ...
  public String generateLLVMIR(ParseTree treeNode) {
    ...
    indentLevel++;
    ParseTree codeNode = treeNode.getChildren().get(3);
    if (codeNode != null) {
  ...
  private String newCondImpl(ParseTree treeNode) {
    ...
    if (children.size() == 1) {
      ...
      String leftId = newCondAtom(children.get(0));
      String rightId = newCondImpl(children.get(2));
  ...
  private String newCondAtom(ParseTree treeNode) {
    ...
    if (children.get(0).getLabel().getValue() == LexicalUnit.PIPE) {
      return newCond(children.get(1));
    } else {
      // CondAtom => CondComp
      return newCondComp(children.get(0));
    }
  }
  ...
  private String newExprArith(ParseTree treeNode) {
    ...
    } else if (child.getLabel().getType() == LexicalUnit.NUMBER) {
      resultId = newAtomValue(child);
    } else {
  ...
  private String newExprArithPrime(ParseTree treeNode) {
    ...
    for (int i = 1; i < children.size(); i += 2) {
      ...
      } else if (children.get(i + 1).getLabel().getValue() == NonTerminal.ExprArithPrime) {
  ...
  private String newProd(ParseTree treeNode) {
    ...
    for (int i = 1; i < children.size(); i += 2) {
      ...
      if (children.get(i + 1).getLabel().getValue() == NonTerminal.Prod) {
        ...
        newExprResult = newAtomValue(children.get(i + 1));
      } else if (children.get(i + 1).getLabel().getValue() == NonTerminal.Atom) {
