...
\begin{enumerate}[label={[\,\arabic*\,]},leftmargin=*,itemsep=4pt]
  ...
  \item \(\langle\text{Cond}\rangle \rightarrow \langle\text{Cond}\rangle\ \rightarrow\ \langle\text{Cond}\rangle\)  % si '->' est l'opérateur, remplacer par \(\to\) ou \(\text{->}\) selon préférence
